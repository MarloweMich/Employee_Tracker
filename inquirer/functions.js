...
async function navFunc() {
  ...
  async function updateEmployee() {
    ...
    const updateInfo = await inquirer.prompt(employeeUpdate);
    db.query(`UPDATE employee SET role_id = ${updateInfo.role} WHERE employee.id = ${updateInfo.employeeID}`,
    (err) => {
      if (err) {
        console.error(
          (`\n Update did not process.`)
        );
        return navFunc();
      } else {
